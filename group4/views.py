...
def adminIndexPage(request):
    if isAdmin(request):
        ...
        requeststudy  = WithdrawStudy.objects.all()
        sums = 0
        change = "-"
        remain = []
        result = []
        ...
        for index in reqcure:
            ...
            if seach != "":
                ...
                change = str(20000 - sums- index.value)
            remain.append(change)
            temp = str(index.value).split('.')
            frontpoint   = temp[0]
            endpoint     = temp[1]
            if len(str(index.value)) >= 6:
                if len(frontpoint) >= 4:
                    endint = frontpoint[-3:]
                    frontint = frontpoint[0:len(frontpoint)-3]
                    newint = frontint+","+endint
            endpoint = endpoint+"0"
            newvalue.append(str(newint)+"."+str(endpoint))
            tempdate = str(index.dateCommit).split(" ")
            month_th = ['มกราคม','กุมภาพันธ์','มีนาคม','เมษายน','พฤษภาคม ','มิถุนายน','กรกฎาคม','สิงหาคม','กันยายน','ตุลาคม','พฤศจิกายน','ธันวาคม']
            month_en = ['January','Febuary','March','April','May','June','July','August','September','October','November','December']
...
def commitWithdrawStudy(request):
    if request.method == "POST":
        objUser         = request.user
        objUsers        = UserProfile.objects.get(user=objUser)
        account_id      = request.POST["account_id"]
...
def addpdf2(request, profID): # use to generate pdf file for lend another teacher.
    ...
    obj = WithdrawStudy.objects.get(pk= int(profID))
